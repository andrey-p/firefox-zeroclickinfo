...
var custombangs = require("./ddg-custombangs");
...
prefSet.on("bang_0_url", onPrefChange);
prefSet.on("bang_0_trigger", onPrefChange);
prefSet.on("bang_1_url", onPrefChange);
prefSet.on("bang_1_trigger", onPrefChange);
prefSet.on("bang_2_url", onPrefChange);
prefSet.on("bang_2_trigger", onPrefChange);
...
function onPrefChange(prefName) {
    ...
    } else if (prefName.indexOf("bang_") === 0) {
        var bangPrefs = [{},{},{}],
            bangId,
            bangProperty;

        for (var prop in prefSet.prefs) {
            if (prop.indexOf("bang_") !== 0) {
                continue;
            }

            bangId = parseInt(prop.match(/_(\d)_/)[1]);
            bangProperty = prop.match(/_(trigger|url)/)[1];

            if (typeof bangId !== "undefined" &&
                    bangProperty !== "undefined" &&
                    prefSet.prefs[prop]) {
                bangPrefs[bangId][bangProperty] = prefSet.prefs[prop];
            }
        }

        custombangs.store(bangPrefs);
    }
}
